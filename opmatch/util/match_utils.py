...
def get_min_cost_flow_dic(edge_ls:List, case_ids:List):
    ...
    # remove all but the case-ncase edges
...
def get_case_ncase_dic(mincostFlow_dic:Dict):
    """From mincostFlow_dic creates a dictionary with case patients as keys
    and corresponding matched control patients as values."""
    case_ncase_dic = dict()
    for ncase in list(mincostFlow_dic.keys()):
        for case in list(mincostFlow_dic[ncase].keys()):
            if case not in case_ncase_dic.keys():
                case_ncase_dic[case] = []
            if mincostFlow_dic[ncase][case]==1:
                case_ncase_dic[case] = case_ncase_dic[case]+[ncase]
...
def matching_dic_from_df(df:pd.DataFrame, matching_ratio:Union[int,str],
    ...
    case_ncase_dic = get_case_ncase_dic(mincostFlow_dic)
    return case_ncase_dic
...
def variable_matching(df):
    ...
    final_case_ncase_dic = {}
    i = 0
    while len(df[(df.case==1) & (df.matched==0)])!=0:
        if i==10:
            print('reached max num of iterations')
            break
        case_ncase_dic = matching_dic_from_df(df, matching_ratio=1)
        
        case_ids = np.array(list(case_ncase_dic.keys()))
        matched_ncase = utils.flatten(list(case_ncase_dic.values()))
        ...
        if i!=0:
            ...
            print(matched_mask)
            case_ncase_dic = {k:v for k,v in case_ncase_dic.items() if k in list(unmatched_inds)}
        final_case_ncase_dic = utils.combine_dicts(final_case_ncase_dic, case_ncase_dic)
        matched_case = case_ids[matched_mask]
        df.matched[matched_case] = 1
        df.matched[matched_ncase] = 1
        avg_dist0 = avg_dist
        i+=1
    return final_case_ncase_dic
...
def match_parallel(ps:np.array, treatment:np.array, matching_ratio:Union[int,str],
    ...
    Returns:
        case_ncase_dic: keys-case patients
    ...
    if isinstance(matching_ratio, int):
        case_ncase_dic = matching_dic_from_df(df, matching_ratio)
        return case_ncase_dic
        ...
        final_case_ncase_dic = variable_matching(df)
        return final_case_ncase_dic
        ...
        case_ncase_dic = matching_dic_from_df(df, 
